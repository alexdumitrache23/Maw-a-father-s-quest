...
// Macro to convert degrees to radians just for GLM functions
...
int main()
{
    ...
    // Build Sewer Walls (WIDENED to 15.0f)
    {
        ...
        glm::vec3 wallScale = glm::vec3(6.0f, 6.0f, 6.0f) / 50.0f;
        ...
        // Back Wall
        GameObject backW;
        backW.pos = glm::vec3(player.pos.x, 2.0f, zStart + 50.0f);
        backW.scale = glm::vec3(15.0f, 15.0f, 1.0f) / 5.0f;
        backW.yaw = 0.0f; backW.type = 10; backW.active = true;
    ...
    auto spawnRat = [&](glm::vec3 p) {
        ...
        GameObject g;
        g.scale = glm::vec3(2.5f);
        g.health = 40.0f; g.active = true;
        g.attacking = false; g.attackCooldown = 2.0f; g.attackTimer = 2.0f; g.type = 1;
        bool placed = false;
        for (int a = 0; a < 30; ++a) {
    ...
    while (!window.isPressed(GLFW_KEY_ESCAPE) && glfwWindowShouldClose(window.getWindow()) == 0)
    {
        ...
        if (state == MENU) {
            ...
            if (ImGui::Button("Restart", ImVec2(200, 60))) {
                ...
                player.pos = glm::vec3(0.0f, 0.5f, 0.0f);
                state = SEWERS;
                enemies.clear(); projectiles.clear(); items.clear(); obstacles.clear();
                ratsSpawned = false; totalSpawned = 0; currentWaveIndex = 0; waveTimer = 0.0f; playerMoved = false;
            }
            ImGui::End();
        }
        else {
            ...
            if (ImGui::Button(topDownView ? "View: Top-Down" : "View: Third-Person", ImVec2(200, 40))) topDownView = !topDownView;
            ImGui::Separator();
            if (state == SEWERS) ImGui::Text("Task: Escape the Sewers (Defeat Rats)");
            else if (state == STREET) ImGui::Text("Task: Avoid Cars & Reach the Restaurant");
            else if (state == MEN_LASAGNA) ImGui::Text("Task: Eat Lasagna & Find Key");
            else if (state == KEY_PUZZLE) ImGui::Text("Task: Grab the Key & Enter Boss Room");
        ...
        if (state != MENU && state != STORY_SCREEN && state != GAME_OVER) {
            ...
            if (state == STREET) {
                ...
                // FLASHLIGHT EFFECT: Light pos = Player Pos
                sunPos = player.pos + glm::vec3(0.0f, 1.0f, 0.0f);
                sunColor = glm::vec3(0.6f, 0.7f, 0.4f); // Sewer Green
                glClearColor(0.05f, 0.05f, 0.05f, 1.0f); // Pitch black background
            }
            else {
                // Interior Lighting
            ...
            if (topDownView) {
                camera.setPosition(player.pos + glm::vec3(0.0f, 20.0f, 1.0f));
                ...
                glm::vec3 baseOffset(0.0f, 4.0f, 8.0f); // Closer camera
            ...
            if (state == SEWERS) {
                ...
                if (player.pos.z < -30.0f) {
                    ...
                    spawnMan(glm::vec3(2.0f, 1.0f, -40.0f));
                    GameObject l; l.pos = glm::vec3(0.0f, 0.5f, -45.0f); l.scale = glm::vec3(0.5f); l.active = true; l.type = 5; l.isHeld = false;
                    items.push_back(l);
                }
            }
                ...
                for (auto& i : items) {
                    if (i.active && i.type == 5 && checkCollision(player, i)) {
                        ...
                        if (menDead) {
                            state = KEY_PUZZLE;
                            items.clear();
                            GameObject k; k.pos = glm::vec3(0.0f, 0.5f, -50.0f); k.scale = glm::vec3(0.3f); k.active = true; k.type = 6; k.isHeld = false;
                            items.push_back(k);
                        }
                    }
                }
                ...
                for (auto& i : items) {
                    if (i.active && i.type == 6 && !i.isHeld && checkCollision(player, i)) i.isHeld = true;
                    if (i.active && i.type == 6 && i.isHeld && player.pos.z < -58.0f) {
                        i.active = false;
                        state = BOSS_RESCUE;
                        items.clear();
            ...
            // --- RENDER 3D SCENE (This was missing in your snippet) ---
            ...
            if (state == SEWERS) {
                ...
                Model = glm::scale(Model, glm::vec3(50.0f, 1.0f, 100.0f));
            ...
            auto DrawMesh = [&](Mesh& m, glm::vec3 pos, glm::vec3 s, float yaw = 0.0f) {
            ...
            DrawMesh(catMesh, player.pos, player.scale, playerYaw);
            if (state != SEWERS) DrawMesh(terrain, glm::vec3(0.0f, -1.5f, 0.0f), glm::vec3(10.0f, 1.0f, 100.0f));
            ...
            for (auto& i : items) if (i.active) {
                if (i.isHeld) {
                    glm::mat4 rot = glm::rotate(glm::mat4(1.0f), TO_RAD(playerYaw), glm::vec3(0.0f, 1.0f, 0.0f));
                    glm::vec3 heldPos = player.pos + glm::vec3(rot * glm::vec4(0.0f, 1.0f, 0.5f, 1.0f));
                    DrawMesh(cube, heldPos, i.scale);
                }
                else {
